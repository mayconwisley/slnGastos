...
{
   public class SaldoPagoPorComp
    {
        Crud crud;
        StringBuilder SQL = null;

        public DataTable ValorPago(int idCliente, int idCompetencia)
        {
            crud = new Crud();
            SQL = new StringBuilder();

            SQL.Append("SELECT SUM(Mov.Valor) AS Pago, STRFTIME('%m/%Y', Comp.Data) AS Data ");
            SQL.Append("FROM Movimentacao Mov ");
            SQL.Append("INNER JOIN Competencia  Comp ON Comp.Id = Mov.CompetenciaId ");
            SQL.Append("WHERE Mov.ClienteId = @ClienteId AND Mov.CompetenciaId <= @CompetenciaId AND Mov.TipoPagoRecebido = 'Pago'");
